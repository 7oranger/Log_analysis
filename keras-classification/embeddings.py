...
def main():
    ...
    parser.add_argument('-d', '--data', help='Path to text files to train on', required=False)
    ...
    parser.add_argument('-n', '--no-train', dest='train', help='Don\'t train embeddings', action='store_false', default=False)
    parser.add_argument('-t', '--tokens', dest='print_tokens', help='Print tokens', action='store_true', default=False)

    options = parser.parse_args()
    options.demo = True
    options.data = r'H:\corpus_trained_model\movie_reviews'
    options.data = r'H:\corpus_trained_model\movie_reviews_mixed\all'
    options.embeddings = r' embeddings.npz'
    options.vocab = r'map.json'
    options.train = True
    options.tokens = True
    ...
    if options.demo:
        ...
        save_path = os.path.join('machiavelli.txt')
